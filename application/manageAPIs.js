            ...
            Promise.all(promises)
              .then(() => {
                return kongAPI.listCertificates()
                  .then((certificates) => {
                    let apisForWhichACertificateIsNeeded = [].concat(newAPIs);
                    certificates.forEach((certificate) => {
                      const domain = certificate.snis[0];
                      let matchingApi = apis.find((api) => return api.route.hosts[0] === domain);
                      if (matchingApi) {
              ...
              .catch((reason) => {
                console.log('Failed to update APIs: ', reason);
                process.exit(0);
              });
          })
          .catch((error) => {
